...
typedef NS_OPTIONS(NSUInteger, NSFileProviderItemCapabilities) {
    ...
    /*
     If a folder is not writable then the UI won't allow creating sub folders,
     importing documents or dropping any item in it.
     */
...
/*
 ...
 When enumerating the root container or a generic container, the parentItemIdentifier of the
 enumerated items is expected to match the enumerated item's identifier.
 When enumerating the working set, the parentItemIdentifier is expected to match the actual
 parent of the item in the hierarchy (ie. it is not NSFileProviderWorkingSetContainerItemIdentifier).
...
/*
 ...
 unix epoch in milliseconds.  If you set the special value 1, the system will
 figure out the best rank and set it:  please persist and sync the new value.
...
/*
 This dictionary should be a property list.  This can be used add state
 information to the item.  FileProviderUI actions predicates can access this
 dictionary.
 */
